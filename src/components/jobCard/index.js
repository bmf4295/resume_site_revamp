...
const JobCard = ({ title, period, company, responsibilities }) => {
    ...
    return (
        <div data-testid="jobCard" className="mb-8 relative">
            ...
            <div ref={cardRef} className="ml-20 p-6 bg-white dark:bg-gray-900 rounded-lg shadow-lg invisible">
                <div className="flex flex-col text-left sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 mb-3">
                    <h3 className="text-xl font-bold dark:text-white">{title}</h3>
                    <span className="text-gray-600 dark:text-gray-400 text-sm sm:text-base sm:text-right">{period}</span>
                </div>
                <div className="text-left">
                    <p className="text-gray-500 dark:text-gray-300 mb-5">{company}</p>
